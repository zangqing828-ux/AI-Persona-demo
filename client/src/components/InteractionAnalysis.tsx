...
export default function InteractionAnalysis({ onComplete }: Props) {
  ...
  const {
    ...
    increment: SIMULATION_CONFIG.PROGRESS.INTERACTION_ANALYSIS.INCREMENT,
    intervalMs: SIMULATION_CONFIG.PROGRESS.INTERACTION_ANALYSIS.INTERVAL_MS,
    maxProgress: SIMULATION_CONFIG.PROGRESS.INTERACTION_ANALYSIS.MAX_PROGRESS,
  });
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {Object.entries(scenarioConfig).map(([key, config]) => {
                  const count = interactionAnalyses.filter(
                    a => a.scenario === key
                  ).length;
                      ...
                      <div className="text-center p-3 bg-muted/50 rounded-lg">
                        ...
                        <p
                          className={`text-xl font-bold ${
                            analysis.repurchaseRate >= 70
                              ? "text-green-600"
                              : analysis.repurchaseRate >= 50
                                ? "text-amber-600"
                                : "text-red-600"
                          }`}
                        >
                        ...
                        {script && (
                          <div>
                            ...
                            >
                              ...
                              <ArrowRight
                                className={`w-4 h-4 ml-1 transition-transform ${showScript ? "rotate-90" : ""}`}
                              />
                            </Button>
