...
export default function DualPersonaGenerator({ onComplete }: Props) {
  ...
  const getSegmentColor = (segment: string) => {
    switch (segment) {
      case '科学养宠': return 'bg-green-500/10 text-green-600 border-green-500/20';
      case '跟风养': return 'bg-blue-500/10 text-blue-600 border-blue-500/20';
      case '穷养': return 'bg-amber-500/10 text-amber-600 border-amber-500/20';
      case '精细养': return 'bg-purple-500/10 text-purple-600 border-purple-500/20';
              ...
              <Card>
                <CardHeader className="pb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <CardTitle className="text-lg">{currentPersona.owner.name}</CardTitle>
                      <CardDescription>
                        {currentPersona.owner.age}岁 · {currentPersona.owner.gender}
                      </CardDescription>
                      <Badge className={`mt-1 ${getSegmentColor(currentPersona.owner.feedingPhilosophy)}`}>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="pt-4 border-t">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        <span className="text-muted-foreground">月均宠物消费</span>
                        <span className="font-medium">{currentPersona.owner.priceRange}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="pt-4 border-t">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {currentPersona.owner.purchaseChannel.map((channel: string, idx: number) => (
                  ...
                  <div className="pt-4 border-t">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {currentPersona.owner.concerns.map((concern: string, idx: number) => (
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {currentPersona.pet.healthStatus.map((status: string, idx: number) => (
                        <Badge key={idx} variant="outline" className="text-xs">
                          {status}
                        </Badge>
                      ))}
                    ...
                    <div className="mt-3 space-y-2">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium">{currentPersona.pet.activityLevel}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-muted-foreground">消化系统</span>
                        <span className="font-medium">{currentPersona.pet.digestiveSystem}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  {currentPersona.pet.allergies.length > 0 && (
                    <div className="pt-4 border-t">
                      <h4 className="text-sm font-medium mb-3 text-amber-600">过敏原</h4>
                      <div className="flex flex-wrap gap-2">
                        {currentPersona.pet.allergies.map((allergy: string, idx: number) => (
                  ...
                  <div className="pt-4 border-t">
                    ...
                    <div className="p-3 bg-muted/50 rounded-lg">
                      <p className="font-medium text-sm">{currentPersona.pet.currentFood}</p>
                    </div>
                  </div>
