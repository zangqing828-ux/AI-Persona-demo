...
export default function InsightDashboard() {
  ...
  // 购买意向分布数据
  const intentData = [
    { name: '高意向', value: batchTestStats.purchaseIntentDistribution.high, color: COLORS.success },
    { name: '中意向', value: batchTestStats.purchaseIntentDistribution.medium, color: COLORS.warning },
    { name: '低意向', value: batchTestStats.purchaseIntentDistribution.low, color: COLORS.danger }
  ];

  // 价格感知分布数据
  const priceData = [
    { name: '便宜', value: batchTestStats.pricePerceptionDistribution.cheap, color: COLORS.success },
    { name: '合理', value: batchTestStats.pricePerceptionDistribution.reasonable, color: COLORS.primary },
    { name: '偏贵', value: batchTestStats.pricePerceptionDistribution.expensive, color: COLORS.warning },
    { name: '太贵', value: batchTestStats.pricePerceptionDistribution.tooExpensive, color: COLORS.danger }
  ];

  // 消化风险分布数据
  const digestiveData = [
    { name: '低风险', value: batchTestStats.digestiveRiskDistribution.low, color: COLORS.success },
    { name: '中风险', value: batchTestStats.digestiveRiskDistribution.medium, color: COLORS.warning },
    { name: '高风险', value: batchTestStats.digestiveRiskDistribution.high, color: COLORS.danger }
  ];

  // 人群分层数据
  const segmentData = batchTestStats.segmentAnalysis.map(seg => ({
    name: seg.segment,
    意向分: seg.avgIntentScore,
    NPS: seg.avgNpsScore + 50, // 调整为正数便于显示
    人数: seg.count
  }));
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            基于 {batchTestStats.totalSamples.toLocaleString()} 份样本的综合分析报告
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {batchTestStats.purchaseIntentDistribution.high.toLocaleString()} 人
            </p>
          </CardContent>
        </Card>
      </div>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div className="text-center p-3 bg-muted/50 rounded-lg">
                      <p className="text-xs text-muted-foreground">平均意向分</p>
                      <p className={`text-xl font-bold ${
                        segment.avgIntentScore >= 60 ? 'text-green-600' :
                        segment.avgIntentScore >= 40 ? 'text-amber-600' : 'text-red-600'
                      }`}>
                    ...
                    <div className="text-center p-3 bg-muted/50 rounded-lg">
                      ...
                      <p className={`text-xl font-bold ${
                        segment.avgNpsScore >= 20 ? 'text-green-600' :
                        segment.avgNpsScore >= 0 ? 'text-amber-600' : 'text-red-600'
                      }`}>
                        {segment.avgNpsScore > 0 ? '+' : ''}{segment.avgNpsScore}
                      </p>
                    </div>
                  </div>
              ...
              <CardContent className="space-y-4">
                <div className="p-4 bg-green-500/5 rounded-lg border border-green-500/20">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-medium text-foreground">产品力获得认可</p>
                      <p className="text-sm text-muted-foreground mt-1">
                        72% 的消费者对产品成分和配方表示认可，"75%动物蛋白"和"无谷配方"是最受欢迎的卖点。
                ...
                <div className="p-4 bg-amber-500/5 rounded-lg border border-amber-500/20">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-medium text-foreground">价格是主要障碍</p>
                      <p className="text-sm text-muted-foreground mt-1">
                        42% 的消费者认为价格偏高，尤其是穷养型人群（意向分仅 35）。
                ...
                <div className="p-4 bg-blue-500/5 rounded-lg border border-blue-500/20">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-medium text-foreground">社交证明是关键转化因素</p>
                      <p className="text-sm text-muted-foreground mt-1">
                        52% 的消费者表示"成分党测评背书"会显著提升购买意愿，
                        48% 希望有试吃装体验。建议加强 KOL 合作和试用装投放。
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex items-start gap-4 p-4 bg-muted/50 rounded-lg">
                    ...
                    <div>
                      <p className="font-medium text-foreground">强化成分党营销</p>
                      <p className="text-sm text-muted-foreground mt-1">
                  ...
                  <div className="flex items-start gap-4 p-4 bg-muted/50 rounded-lg">
                    ...
                    <div>
                      <p className="font-medium text-foreground">推出试吃装策略</p>
                      <p className="text-sm text-muted-foreground mt-1">
                  ...
                  <div className="flex items-start gap-4 p-4 bg-muted/50 rounded-lg">
                    ...
                    <div>
                      <p className="font-medium text-foreground">差异化人群策略</p>
                      <p className="text-sm text-muted-foreground mt-1">
                        科学养宠型：强调成分优势和功效数据；
                        跟风养型：加强社交证明和熟人推荐；
                        穷养型：考虑推出性价比子品牌或学生优惠。
                  ...
                  <div className="flex items-start gap-4 p-4 bg-muted/50 rounded-lg">
                    ...
                    <div>
                      <p className="font-medium text-foreground">建立效果追踪机制</p>
                      <p className="text-sm text-muted-foreground mt-1">
